...
bic.high.expression.heatmap <- function(cds, num.gns=100, transform = TRUE, idMap = NULL, 
    ...
    if(transform){
        dat <- cds %>% 
               DESeq::estimateDispersions(method="blind", sharingMode = "fit-only", fitType = fitType) %>%
...
#' 
#' @export
bic.sample.to.sample.distances <- function(cds, key = NULL, clrs = NULL, file=NULL, width = 8.5, height = 11, fitType = "parametric"){
  
    dat <- cds %>%
           estimateDispersions(method = "blind", sharingMode = "fit-only", fitType = fitType) %>%
           varianceStabilizingTransformation() %>%
...
bic.deseq.plot.pca <- function(cds, file=NULL, ntop = 500, labels = FALSE, clrs = NULL, idMap = NULL,
                               height = 8.5, width = 11, fitType = "parametric", method = "blind"){
  cdsBlind <- estimateDispersions(cds, method = "blind", fitType = fitType)
...
bic.plot.pc.loading <- function(cds, plotPCs = c("PC1", "PC2"), ntop = 500, fitType = "parametric", file = NULL,
...
prep.for.volcano <- function(res, pvalCol, fcCol, pCut, fcCut, labelCol = NULL){
    ...
    ## set any Inf value to one more than the maximum real value 
    max <- max(vdat$`-Log10P`[vdat$`-Log10P` != Inf])
    vdat$`-Log10P`[vdat$`-Log10P` == Inf] <- max + 1
...
bic.volcano.plot <- function(dat, fcCol, pvalCol = "pval", maxSig = 0.05, fcCut = 1, title = "",
                              yLabel = "Significance", xLabel = "Fold Change", pointLabelCol = "plotLabel", 
                              labelSelection = "both", maxUpLabels = 25, maxDnLabels = 25, showCutoffs = TRUE, file = NULL){
    ...
    if(!showCutoffs){
        plotDat$Significance <- NA #"NS"
        clrs <- c(NS = "black")
        lgndLabels <- NULL #c(NS = "NS")
    }
...
legendDir <- "vertical"
    ...
    ### build plot
    p <- ggplot(plotDat, 
                aes(x = !!as.name(fcCol), y = !!as.name(pvalCol), 
    ...
    p <- p +
         ...
         scale_color_manual(values = clrs, labels = lgndLabels, na.value = "black") 
    ...
    # select and label genes
    if(labelSelection != 'none' & !is.na(pointLabelCol)){
        ranked <- plotDat %>%
                  filter(!is.na(!!sym(pointLabelCol))) %>%
                  mutate(upFcRank = row_number(-!!sym(fcCol)),
                         dnFcRank =  row_number(!!sym(fcCol)),
                         pRank = row_number(-!!sym(pvalCol)), ## todo: rank up and down gns separately?
                         minUpRank = pmin(upFcRank, pRank),
                         minDnRank = pmin(dnFcRank, pRank)) 

        selCols = list(both   = c("minUpRank", "minDnRank"),
                       fc     = c("upFcRank", "dnFcRank"),
                       signif = c("pRank", "pRank")) 

        upCol <- selCols[[labelSelection]][1]
        dnCol <- selCols[[labelSelection]][2]

        labelDat <- ranked %>% 
                    filter(!!sym(upCol) <= maxUpLabels | 
                           !!sym(dnCol) <= maxDnLabels)
        p <- p +
             geom_text_repel(data = labelDat,
                             aes(x = !!as.name(fcCol), 
                                 y = !!as.name(pvalCol), 
                                 label=!!as.name(pointLabelCol)),
                             color = "black",
...
#' 
#' @export
bic.expression.heatmap <- function(norm.counts, genes, file = NULL, annotate_samples = TRUE,
    ...
    colAnnot <- NULL
    if(annotate_samples && !is.null(key)){
        if(is.null(annClrs)){
...
bic.rnaseq.heatmap <- function(htmpDat, cols, rows, clusterCols = TRUE, clusterRows = TRUE, colAnnot = NULL, 
    ...
    if(nrow(lgndDat) > 0){
        lgndDat <- lgndDat %>% dplyr::select(Val) %>% unique %>% mutate(x = 1, y = rev(row_number()))
...
bic.deseq.qc <- function(cds, conds, clustering.dir, idMap = NULL, fitType = "parametric"){
    tmp <- tryCatch({
             capture.output(
