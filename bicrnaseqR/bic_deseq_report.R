...
get.deseq.plots <- function(compSetRes, request, svnrev, pre = "TEMP", idMap = NULL, maxPlots = 9, maxLabels = 25, 
                             key = NULL, annColors = NULL, fitType = "parametric", deHtmpOpts = NULL, volcanoOpts = NULL, 
                             clusterDir = NULL, deFigDir = NULL, qcDir = NULL,
    ...
    ## PCA & loadings    
    log_debug("PCA")
    report$`PCA`       <- bic.deseq.plot.pca(cds, labels = FALSE, clrs = annColors, fitType = fitType) +
                          xtheme
    if(labelPlots){
        report$`PCA (samples labeled)` <- bic.deseq.plot.pca(cds, labels = TRUE, clrs = annColors, fitType = fitType) +
                                          xtheme
    }

    log_debug("PC loadings")
    report$`PC loadings` <- bic.plot.pc.loading(cds, idMap = idMap, fitType = fitType)
    ...
    report$`Expression of top 50 genes` <- bic.high.expression.heatmap(cds, transform = TRUE, 
                                                                       ...
                                                                       key = key, clrs = annColors, fitType = fitType) 

    ## sample to sample   
    log_debug("sample to sample distances")
    report$`Sample to sample distances` <- bic.sample.to.sample.distances(cds, key = key, clrs = annColors, fitType = fitType) 
    ...
    for(comp in comps){
        ...
        p1 <- prep.for.volcano(dat, "pval", fcCol, pCut, minFC) %>%
              ...
              theme(#legend.position = c(-0.05, 1.15),
                    #legend.text = element_text(margin = unit(c(0, 1, 0, 0), "in")),
        ...
        if(is.null(compSetRes[[comp]]$DE$filtered) || nrow(compSetRes[[comp]]$DE$filtered) < 3 ){
            ...
            genes <- compSetRes[[comp]]$DE$filtered %>% 
                     arrange(desc(abs(!!as.name(fcCol)))) %>%
                     pull(GeneSymbol)
            genes <- genes[1:min(50, length(genes))]
            htmpDat <- compSetRes$norm.counts %>% 
                       select(GeneSymbol, paste(compKey$Sample, compKey$Group, sep="__"))
            report[[comp]]$`DE heatmap` <- bic.expression.heatmap(htmpDat,
                                                                  genes, 
                                                                  key = compKey, 
...
bic.deseq.report <- function(compSetRes, request, svnrev, pdfFile, pre = "TEMP", maxPlots = 9, idMap = NULL, 
    ...
    report <- tryCatch({
                  get.deseq.plots(compSetRes, request, svnrev, pre = pre, idMap = idMap, maxPlots = maxPlots, 
                                   key = key, fitType = fitType, annColors = annColors, deHtmpOpts = deHtmpOpts,
                                   volcanoOpts = volcanoOpts, 
                                   clusterDir = clusterDir, deFigDir = deFigDir, qcDir = qcDir, 
                                   cellCompDir = cellCompDir)
                }, error = function(){
