bic.log.params <- function(pre, forceRerun, cibersort, diff.exp, GSA, GSEA, report, counts.file, cibersortR,
    ...
    log_debug("  singularity:   ", singularity)
    log_debug("  gsea.image:    ", gsea.image)
...
bic.setup.directories <- function(counts.dir, clustering.dir, all.gene.dir = NULL,
    ...
    if(multi.comps){
        for(i in comp.set.nums){
            ...
            if(!is.null(cibersort.dir)){
                dirs$cellCompDir = file.path(dirname(cibersort.dir), setName, paste0(basename(cibersort.dir), sfx))
            }
            if(!is.null(diff.exp.dir)){
...
bic.qc.comparison.set <- function(key, comps){
    ...
    ec <- 0
    conds <- unique(unlist(strsplit(comps, " - ")))
    keyNotComps <- setdiff(unique(key$Group), conds)
    compsNotKey <- setdiff(conds, unique(key$Group))
...
bic.setup.comparisons.from.standard.files <- function(keyFile, compFile){
    ...
    key   <- read.csv(keyFile, header = F, sep = "\t", check.names = F, strip.white = T) %>%
             as_tibble() %>%
             select_if(~!all(is.na(.))) %>%
    ...
    comps <- read.csv(compFile, header = F, sep = "\t", check.names = F, strip.white = T)
...
bic.setup.comparisons.from.multi.column.files <- function(keyFile, compFile){
    ...
    keys <- read.csv(keyFile, header = T, sep = "\t", check.names = F, strip.white = T)   ## Sample Comp_1 Comp_2
    keys <- keys[complete.cases(keys),]
    comps <- read.csv(compFile, header = T, sep = "\t", check.names = F, strip.white = T) ## CompSet  Group1   Group2
...
bic.singularity.command <- function(singularity = "", image = "", bind = getwd()){
    cmd <- ""
    sing.params <- c(singularity, image)
    if(!all(sing.params == "")){
       if(any(sing.params == "")){
           stop("Must provide BOTH or NEITHER params 'singularity' AND 'singularity.image'")
       }
       cmd <- paste(singularity, "exec", "--bind", bind, "--pwd", getwd(), image)
    }
    cmd
}
