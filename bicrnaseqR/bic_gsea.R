...
bic.run.gsea <- function(ncounts, key, comp, gsea.sh, reportLabel, outDir,
    ...
    ec <- system(cmd)
    if(ec == 0){
        ## returns true or false, or NULL if multiple directories found
...
bic.gsea.per.collection <- function(collections, ncounts, key, comp, gsea.sh, reportLabel, outDir, 
    ...
    lapply(tolower(collections), function(coll){
        ...
        ec <- bic.run.gsea(ncounts, key, comp, gsea.sh, reportLabel, outDir,
...
bic.gsea.preranked.per.collection <- function(collections, dat, gmt.files, statCol, gsea.sh, reportLabel, outDir,
     ...
     lapply(tolower(collections), function(coll){
        ...
        ec <- bic.run.gsea.preranked(dat, statCol, gsea.sh, reportLabel, outDir,
                                     gmt.files = gmt_file, gs.min = gs.min,
                                     gs.max = gs.max, 
                                     singularity = singularity)
        if(ec != 0){                                                            
            log_error("An error occurred while running GSEA Preranked for comparison [", reportLabel, "].")
        } else {                                                                
            log_info("Done!")                                                   
        }                                                                       
        ec                                                                      
...
which.gsea.version <- function(key, comps, delim = " - "){
    conds <- strsplit(comps, delim) %>% unlist %>% unique
    smallest <- key %>%
                filter(Group %in% conds) %>%
                group_by(Group) %>%
                summarize(size = n()) %>%
                pull(size) %>%
                min
    ifelse(smallest < 3, "preranked", "gsea")
}
