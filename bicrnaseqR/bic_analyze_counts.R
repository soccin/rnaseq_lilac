...
bic.format.htseq.counts <- function(htseq.file,key=NULL){
  ...
  HTSeq.dat <- read.csv(htseq.file, header = T, sep = "\t", check.names = F, strip.white = T) %>%
               as_tibble()
...
bic.get.deseq.cds <- function(raw.counts, conds,
  ...
  ids <- counts.tmp$ID
  mat <- as.matrix(counts.tmp[,-1]) + 1  ## ADD ONE IN CASE EVERY GENE HAS AT LEAST ONE ZERO, WHICH CAUSES ERROR IN estimateSizeFactors()
  rownames(mat) <- ids
  ...
  cds <- tryCatch({
           DESeq::estimateDispersions(cds, fitType=fitType, method=method, sharingMode=sharingMode)
         }, error = function(err){
           if(!method == "pooled"){
             log_warn(paste("method='",method,"' did not work. Trying method='pooled'",sep=""))
             cds <- tryCatch({
                  DESeq::estimateDispersions(cds, fitType=fitType, method='pooled', sharingMode=sharingMode)
               }, error = function(x){
                  if(!method == "blind"){
                    log_warn(paste("method='",method,"' did not work. Trying method='blind'",sep=""))
                    cds <- tryCatch({
                         DESeq::estimateDispersions(cds, fitType=fitType, method='blind', sharingMode='fit-only')
                    }, error = function(x){
                         log_warn(paste("All methods '",method,"', 'pooled' and 'blind' failed.",sep=""))
                         return(NULL)
                    })
                  }
               })
...
bic.standard.clustering <- function(cds, scaled.counts, out.dir, idMap = idMap, pre = "TEMP", annColors = NULL, fitType = "parametric"){
    ...
    tryCatch({
        tmp <- bic.deseq.plot.pca(cds, file = file.name, labels = FALSE, clrs = annColors, fitType = fitType)
        tmp <- bic.deseq.plot.pca(cds, file = file.name2, labels = TRUE, clrs = annColors, fitType = fitType)
