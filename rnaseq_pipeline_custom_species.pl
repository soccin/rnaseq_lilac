...
my $email = "$uID\@mskcc.org";
my $rsync = "/lila/data/bicgrp/share/depot/assemblies/res/bic/$uID";
...
GetOptions ('map=s' => \$map,
	    ...
	    'r2adaptor=s' => \$r2adaptor,
 	    ...
 	    'priority_group=s' => \$priority_group,
            ...
            'qc_bed=s' => \$qc_bed,
            'id_map=s' => \$id_map,
            'ribosomal_intervals=s' => \$ribosomal_intervals) or exit(1);
...
elsif($ENV{'LSF_ENVDIR'} eq "/common/lila/data/bicgrp/share/depot/assemblies/OS7/conf" or $ENV{'LSF_ENVDIR'} eq "/admin/lsfjuno/lsf/conf"){
    $scheduler = 'juno';
}
elsif($ENV{'SGE_ROOT'} ne ""){
    $scheduler = 'sge';
}
elsif($ENV{'LSF_ENVDIR'} eq "/admin/lsflilac/lsf/conf"){
    $scheduler = 'lilac';
}
else{
...
if($refseq){
    $commandLine .= " -refseq $refseq";
}
if($star_index){
    $commandLine .= " -star_index $star_index";
}
if($qc_bed){
    $commandLine .= " -qc_bed $qc_bed";
}
if($id_map){
    $commandLine .= " -id_map $id_map";
}
if($ribosomal_intervals){
    $commandLine .= " -ribosomal_intervals $ribosomal_intervals";
}
$commandLine .= "\n";
...
## this check messes things up because of the way permissions are set up on /igo.
## the permissions on the sample directory causes this 'die' to be executed, but the
## fastqs in that directory can still be read 
    if(!-d $data[3]){
...
#my $Rlibs = '';
### TEMPORARILY HARD CODE Rlibs TO GET DE MODULE TO RUN ON LILAC
my $Rlibs = "$Bin/lib/R-4.0.0/";
#################################################################
#
my %samp_libs_run = ();
my $slr_count = 0;
...
if($species =~ /custom/i){
    ### assume custom species
    ### TODO: add validation of these required params
    $REF_SEQ = $refseq;
    $GTF = $gtf;
    $QC_BED = $qc_bed;
    if($r1adaptor){
        $starDB = "$star_index/overhang49";
    }
    else {
        $starDB = "$star_index/overhang74";
    }
    $geneNameConversion = $id_map;
...
elsif($species =~ /human|hg19/i){
    $species = 'hg19';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/hg19.fasta';
    $DEXSEQ_GTF = "$Bin/data/human/gencode.v18.annotation_dexseq.gtf";
    $QC_BED = "$Bin/data/hg19/hg19_GENCODE_GENE_V19_comprehensive.bed";
    $CHIMERASCAN_INDEX = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/chimerascan/0.4.5a';
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/bowtie/1.0.0/hg19_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/bowtie/2.2.4/hg19_bowtie2';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/chromosomes';
    $RIBOSOMAL_INTERVALS = "$Bin/data/hg19/ribosomal_hg19.interval_file";
    $REF_FLAT = "$Bin/data/hg19/refFlat__hg19.txt.gz";
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/bowtie/2.2.4/transcriptome/gencode/v18/gencode.v18.annotation';
    $TRANS_INDEX_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/bowtie/2.2.4/transcriptome/gencode/v18/deduplicated/gencode.v18.annotation.dedup';
    $TRANS_FASTA_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/bowtie/2.2.4/transcriptome/gencode/v18/deduplicated/gencode.v18.annotation.dedup.fa';
    $geneNameConversion = "$Bin/data/human/gencode18IDToGeneName.txt";
    $KALLISTO_INDEX = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/kallisto/v0.42.1/gencode/v18/gencode.v18.annotation.gtf.fasta.idx';
    ...
    if($lncrna){
        ...
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/star/2.3.0e_r291/LNCipedia';
    } else {
        $GTF = "$Bin/data/human/gencode.v18.annotation.gtf";
	if($r1adaptor){
	    $starDB = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/star/2.4.1d/gencode/v18/overhang49';
	}
	else{
	    $starDB = '/lila/data/bicgrp/share/depot/assemblies/H.sapiens/hg19/index/star/2.4.1d/gencode/v18/overhang74';
	}
    }

    $STAR_MAX_MEM = 35;
}
elsif($species =~ /mouse|mm10/i && $species !~ /custom/i){
    $species = 'mm10';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/mm10.fasta';
    ...
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/bowtie/1.1.1/mm10_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/bowtie/2.2.4/mm10_bowtie2';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/chromosomes';
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/bowtie/2.2.4/transcriptome/gencode/vM8/gencode.vM8.annotation';
    $TRANS_INDEX_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/bowtie/2.2.4/transcriptome/gencode/vM8/deduplicated/gencode.vM8.annotation.dedup';
    $TRANS_FASTA_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/bowtie/2.2.4/transcriptome/gencode/vM8/deduplicated/gencode.vM8.annotation.dedup.fa';
    $RIBOSOMAL_INTERVALS = "$Bin/data/mm10/ribosomal_mm10.interval_file";
    $REF_FLAT = "$Bin/data/mm10/refFlat__mm10.txt.gz";
    $KALLISTO_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/kallisto/v0.42.1/gencode/vM8/gencode.vM8.annotation.gtf.fasta.idx';
    $RSEM_DB = '/opt/common/CentOS_6/rsem/RSEM-1.2.25/data/GRCm38/star/GRCm38_vM8_gencode';
    $GMT_DIR = "$Bin/data/mouse/MSigDB/v7.1";
    if($r1adaptor){
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/star/2.4.1d/gencode/vM8/overhang49';
    }
    else{
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm10/index/star/2.4.1d/gencode/vM8/overhang74';
    }
    ...
    $species = 'mm9';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/mm9.fasta';
    ...
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/index/bowtie/1.0.0/mm9_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/index/bowtie/2.1.0/mm9_bowtie2';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/chromosomes';
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/index/bowtie/2.1.0/transcriptome/ensembl/vTBD/ensembl';
    $TRANS_INDEX_DEDUP = '';
    $TRANS_FASTA_DEDUP = '';
    ...
    $GMT_DIR = "$Bin/data/mouse/MSigDB/v7.1";
    if($r1adaptor){
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/index/star/2.4.1d/ensembl/v67/overhang49';
    }
    else{
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/M.musculus/mm9/index/star/2.4.1d/ensembl/v67/overhang74';
    }
    ...
    $species = 'hybrid';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/hg19_mm10.fasta';
    $GTF = "$Bin/data/human/gencode.v18.annotation.gtf";
    $DEXSEQ_GTF = "$Bin/data/human/gencode.v18.annotation_dexseq.gtf";
    $geneNameConversion = "$Bin/data/human/gencode18IDToGeneName.txt";
    $RIBOSOMAL_INTERVALS = "$Bin/data/hg19_mm10/ribosomal_hg19_mm10.interval_file";
    $REF_FLAT = "$Bin/data/hg19/refFlat__hg19.txt.gz";
    $RSEM_DB = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/rsem/1.2.25/gencode/v18/hg19_mm10';
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/bowtie/1.1.1/hg19_mm10_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/bowtie/2.2.4/hg19_mm10_bowtie2';
    $KALLISTO_INDEX = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/kallisto/v0.42.1/gencode/v18/gencode.v18.annotation.gtf.fasta.idx';
    $STAR_FUSION_GENOME_LIB = "$STAR_FUSION/Hg19_CTAT_resource_lib";
    $CHIMERASCAN_INDEX = "/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/chimerascan/0.4.5a";
    $CHIMERASCAN_FP_FILTER = "$Bin/data/hg19/hg19_bodymap_false_positive_chimeras.txt";
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/chromosomes';
    $QC_BED = "$Bin/data/hg19/hg19_GENCODE_GENE_V19_comprehensive.bed";
    $GMT_DIR = "$Bin/data/human/MSigDB/v7.1";
    if($r1adaptor){
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/star/2.4.1d/gencode/v18/overhang49';
    }
    else{
	$starDB = '/lila/data/bicgrp/share/depot/assemblies/hybrids/H.sapiens_M.musculus/hg19_mm10/index/star/2.4.1d/gencode/v18/overhang74';
    }
    ...
    $species = 'Rn6';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/Rnor_6.0.fasta';
    $GTF = "$Bin/data/rn6/Rattus_norvegicus.Rnor_6.0.92.gtf";

    $geneNameConversion = "$Bin/data/rn6/rn6Ensembl92IDToGeneName.txt";


    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/bowtie/1.1.1/Rnor_6.0_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/bowtie/2.2.4/Rnor_6.0_bowtie2';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/chromosomes';
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/bowtie/2.2.4/transcriptome/ensembl/v92/Rattus_norvegicus.Rnor_6.0.92';
    $TRANS_INDEX_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/bowtie/2.2.4/transcriptome/ensembl/v92/deduplicated/Rattus_norvegicus.Rnor_6.0.92.gtf.dedup';
    $TRANS_FASTA_DEDUP = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/bowtie/2.2.4/transcriptome/ensembl/v92/deduplicated/Rattus_norvegicus.Rnor_6.0.92.gtf.dedup.fa';
    ...
    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/star/2.4.1d/ensembl/v92/overhang49';
    }
    else{
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/R.norvegicus/Rnor_6.0/index/star/2.4.1d/ensembl/v92/overhang74';
    }
    ...
    $species = 'z11';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz11/GRCz11.fasta';
    $GTF = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz11/Danio_rerio.GRCz11.97.gtf';
    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz11/index/star/2.4.1d/ensembl/v97/overhang49';
    } else {
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz11/index/star/2.4.1d/ensembl/v97/overhang74';
    }
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz11/chromosomes';
    ...
    $species = 'zv10';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz10/GRCz10.fasta';
    $GTF = "$Bin/data/zv10/GRCz10.gtf";
    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz10/index/star/2.5.0a/ensembl/v83/overhang49';
    } else {
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz10/index/star/2.5.0a/ensembl/v83/overhang74';
    }
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/GRCz10/chromosomes';
    ...
    $species = 'zv9';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/zv9/zv9.fasta';
    $GTF = "$Bin/data/zv9/zv9.gtf";
    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/zv9/index/star/2.4.1d/ensembl/v79/overhang49';
    } else {
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/zv9/index/star/2.4.1d/ensembl/v79/overhang74';
    }
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/D.rerio/zv9/chromosomes';
    ...
    $species = 'dm6';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/dm6.fasta';
    $GTF = "/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/dmel-all-r6.33.gtf";
    $QC_BED = "/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/dm6_refseq_all.bed";
    $DEXSEQ_GTF = "";
    $geneNameConversion = "/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/IDToGeneName.txt";
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/index/bowtie/1.1.1/dm6_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/index/bowtie/2.2.4/dm6_bowtie2';
    $RSEM_DB = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/index/rsem/1.3.3/flybase/6.33/star/dm6';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/chromosomes';
    $TRANS_INDEX = '';
    $TRANS_INDEX_DEDUP = '';
    $TRANS_FASTA_DEDUP = '';
    $RIBOSOMAL_INTERVALS = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/ribosomal_dm6.interval_file';
    $REF_FLAT = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/dm6_refFlat.txt';
    $KALLISTO_INDEX = '';

    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/index/star/2.4.1d/flybase/6.33/overhang49';
    }
    else{
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/D.melanogaster/dm6/index/star/2.4.1d/flybase/6.33/overhang74';
    }
    ...
    $species = 'WBcel235';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/WBcel235.fasta';
    $GTF = "$Bin/data/WBcel235/ensembl_WBcel235.gtf";
    $DEXSEQ_GTF = '';
    $CHIMERASCAN_INDEX = '';
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/bowtie/1.1.1/WBcel235_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/bowtie/2.2.4/WBcel235_bowtie2';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/chromosomes';
    $RIBOSOMAL_INTERVALS = "$Bin/data/WBcel235/ribosomal_WBcel235.interval_file";
    $REF_FLAT = "$Bin/data/WBcel235/refFlat__ce11.txt.gz";
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/bowtie/2.2.4/transcriptome/ensembl/v20151123/genes';
    $TRANS_INDEX_DEDUP = '';
    $TRANS_FASTA_DEDUP = '';
    $geneNameConversion = "$Bin/data/WBcel235/WBcel235EnsemblIDToGeneName.txt";
    $KALLISTO_INDEX = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/kallisto/v0.42.1/ensembl/v20151123/genes.gtf.fasta.idx';
    $RSEM_DB = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/rsem/1.2.25/star/2.4.1d/WBcel235_ensembl';
    $STAR_FUSION_GENOME_LIB = '';
    $CHIMERASCAN_FP_FILTER = '';

    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/star/2.4.1d/ensembl/v20151123/overhang49';
    }
    else{
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/custom/C.elegans/WBcel235/index/star/2.4.1d/ensembl/v20151123/overhang74';
    }
    ...
    $species = 'S288C_R64';
    $REF_SEQ = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/S288C_R64.fasta';
    $GTF = '/lila/data/bicgrp/share/depot/assemblies/depot/annotation/S.cerevisiae/ensembl/v94/Saccharomyces_cerevisiae.R64-1-1.94.gtf';
    $QC_BED = "$Bin/data/S288C_R64/Saccharomyces_cerevisiae.R64-1-1.94.bed";
    $DEXSEQ_GTF = '';
    $CHIMERASCAN_INDEX = '';
    $BOWTIE_INDEX = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/bowtie/1.1.1/S288C_R64_bowtie';
    $BOWTIE2_INDEX = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/bowtie/2.2.4/S288C_R64_bowtie';
    $chrSplits = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/chromosomes';
    ...
    $TRANS_INDEX = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/bowtie/2.2.4/transcriptome/ensembl/v94/Saccharomyces_cerevisiae.R64-1-1.94';
    $TRANS_INDEX_DEDUP = '';
    $TRANS_FASTA_DEDUP = '';

    $geneNameConversion = "$Bin/data/S288C_R64/S288C_R64EnsemblIDToGeneName.txt";

    $KALLISTO_INDEX = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/kallisto/v0.42.1/ensembl/v94/Saccharomyces_cerevisiae.R64-1-1.94';
    $RSEM_DB = '';
    ...
    if($r1adaptor){
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/star/2.4.1d/ensembl/v94/overhang49';
    }
    else{
        $starDB = '/lila/data/bicgrp/share/depot/assemblies/S.cerevisiae/S288C_R64/index/star/2.4.1d/ensembl/v94/overhang74';
    }
...
if($deseq || $clustering_only){
    ...
    my $qc = `$R/Rscript $Bin/RunDE_inputQC.R --bin $Bin --Rlibs $Rlibs --pre $pre $cluster_only 2>&1`;
    my $ec = $? >> 8;
    print("$qc");
    if($ec == 1){
...
my $standardParams = Schedule::queuing(%stdParams);
	...
	if($htseq || $dexseq){
		...
		if(!-e "$output/progress/$pre\_$uID\_HT_STAR_$sample.done" || $ran_starqns){
		    ...
		    my $standardParams = Schedule::queuing(%stdParams);
		    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s no -t exon $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample.htseq_count"`;
		    `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_$sample.done`;
		    push @starhtseq_jids, "$pre\_$uID\_HT_STAR_$sample";
		    $ran_starhtseq = 1;
		}

                ### TEMPORARY FOR PROJ_11875_BC
                #`/bin/mkdir -m 775 -p $output/gene/counts_gene`;
                #if(!-e "$output/progress/$pre\_$uID\_HT_STAR_PROMPTS_$sample.done" || $ran_starqns){
                #    sleep(3);
                #    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_PROMPTS_$sample", job_hold => "$starqnsj", cpu => "1", mem => "1", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_PROMPTS_$sample.log");
                #    my $standardParams = Schedule::queuing(%stdParams);
                #    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s $htseq_stranded -t PROMPT $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample\_PROMPT.htseq_count"`;
                #    `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_PROMPTS_$sample.done`;
                #    push @starhtseq_jids, "$pre\_$uID\_HT_STAR_PROMPTS_$sample";
                #}
                ### TEMPORARY FOR PROJ_11875_BC
                #`/bin/mkdir -m 775 -p $output/gene/counts_gene`;
                #if(!-e "$output/progress/$pre\_$uID\_HT_STAR_PAXT_$sample.done" || $ran_starqns){
                #    sleep(3);
                #    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_PAXT_$sample", job_hold => "$starqnsj", cpu => "1", mem => "1", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_PAXT_$sample.log");
                #    my $standardParams = Schedule::queuing(%stdParams);
                 #   `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s $htseq_stranded -t PAXT $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample\_PAXT.htseq_count"`;
                 #   `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_PAXT_$sample.done`;
                 #   push @starhtseq_jids, "$pre\_$uID\_HT_STAR_PAXT_$sample";
                #}
                ### TEMPORARY FOR PROJ_11875_BC
                #`/bin/mkdir -m 775 -p $output/gene/counts_gene`;
                #if(!-e "$output/progress/$pre\_$uID\_HT_STAR_NEXT_$sample.done" || $ran_starqns){
                #    sleep(3);
                #    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_NEXT_$sample", job_hold => "$starqnsj", cpu => "1", mem => "1", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_NEXT_$sample.log");
                #    my $standardParams = Schedule::queuing(%stdParams);
                #    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s $htseq_stranded -t NEXT $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample\_NEXT.htseq_count"`;
                #    `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_NEXT_$sample.done`;
                #    push @starhtseq_jids, "$pre\_$uID\_HT_STAR_NEXT_$sample";
                #}
                ### TEMPORARY FOR PROJ_11875_BC
                #`/bin/mkdir -m 775 -p $output/gene/counts_gene`;
                #if(!-e "$output/progress/$pre\_$uID\_HT_STAR_L1_$sample.done" || $ran_starqns){
                #    sleep(3);
                #    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_L1_$sample", job_hold => "$starqnsj", cpu => "1", mem => "1", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_L1_$sample.log");
                #    my $standardParams = Schedule::queuing(%stdParams);
                #    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s $htseq_stranded -t L1 $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample\_L1.htseq_count"`;
                #    `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_L1_$sample.done`;
                #    push @starhtseq_jids, "$pre\_$uID\_HT_STAR_L1_$sample";
                #}
                ### TEMPORARY FOR PROJ_11875_BC
                #`/bin/mkdir -m 775 -p $output/gene/counts_gene`;
                #if(!-e "$output/progress/$pre\_$uID\_HT_STAR_eRNA_$sample.done" || $ran_starqns){
                #    sleep(3);
                #    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_eRNA_$sample", job_hold => "$starqnsj", cpu => "1", mem => "1", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_eRNA_$sample.log");
                #    my $standardParams = Schedule::queuing(%stdParams);
                #    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams "$singularityParams $PYTHON/python $HTSEQ/htseq-count -m intersection-strict -s no -t eRNA $starOut\_queryname_sorted.sam $GTF > $output/gene/counts_gene/$sample\_eRNA.htseq_count"`;
                #    `/bin/touch $output/progress/$pre\_$uID\_HT_STAR_eRNA_$sample.done`;
                #    push @starhtseq_jids, "$pre\_$uID\_HT_STAR_eRNA_$sample";
                #}
...
if($clustering_only){
    ...
    if($tophat && $htseq){
        ...
        if(!-e "$output/progress/$pre\_$uID\_SAMPLE_CLUSTERING_TOPHAT2.done" || $ran_shmatrix){
            sleep(3);
    ...
    if($kallisto){
        ...
        if(!-e "$output/progress/$pre\_$uID\_SAMPLE_CLUSTERING_KALLISTO.done" || $ran_shmatrix){
            sleep(3);
    ...
    if($rsem){
        ...
        if(!-e "$output/progress/$pre\_$uID\_SAMPLE_CLUSTERING_RSEM.done" || $ran_shmatrix){
            sleep(3);
    ...
    if($species =~ /hybrid|hg19|human/i){
        $deseq_species = "human";
    }
...
} # end if clustering_only else if deseq 
...
#		die "CAN'T FIND cutadapt IN $conf[1] $!";
    ...
    $ENV{'LD_LIBRARY_PATH'} = "/opt/common/CentOS_7/gcc/gcc-7.3.0/lib64:$ENV{'LD_LIBRARY_PATH'}";
