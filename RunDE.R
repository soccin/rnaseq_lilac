...
usage <- function(){
  ...
  [OPTIONAL]
    ...
    --singularity    default = \"\"; if using any singularity images (currently this code only supports
                     the use of a GSEA image), this is the path to singularity
    --singularity.bind default = $PWD; comma-delimited string containing directories to pass to the '--bind' 
                     parameter of singularity's 'exec' command
    --gsea.image     default = \"\"; path to a GSEA singularity image
...
### need to make sure these packages are loaded from the R being run
...
print(".libPaths() before loading source_bicrnaseq.R: ")
print(.libPaths())
...
## determine which version of GSEA to run: if one or more comparisons has fewer than
## two samples in any group, need to run GSEAPreranked
if(GSEA){
    GSEAver <- "gsea"
    verPerComp <- lapply(compSetNums, function(x){ 
                    which.gsea.version(keysAndComps$keys[[x]],
                                       keysAndComps$comparisons[[x]])
                  }) %>%
                  unlist
    if(any(verPerComp == "preranked")){
        log_warn("Running *** GSEAPreranked ***!!! One or more comparisons includes a sample group of fewer than three samples. Using fold changes from DESeq to run GSEA instead of normalized counts.")
        GSEAver <- "preranked"
    }
}
...
parLapply(cl, compSetNums, function(compSet){
    ...
    if(diff.exp){
        ...
        for (comp in comps){
            ...
            condA <- unlist(strsplit(compName, "_vs_"))[2]
            condB <- unlist(strsplit(compName, "_vs_"))[1]
            ...
            if(GSEA){
                singularity_command <- bic.singularity.command(singularity = singularity, image = gsea.image, bind = singularity.bind)
                if(is.null(all_results[[compName]]$GSEA) || forceRerun){
                    success <- 0
                    if(GSEAver == "preranked"){
                        log_info("Running GSEA Preranked...\n")
                        fcCol <- paste0("log2[", gsub("_vs_", "/", compName), "]")  
                        allRes <- all_results[[compName]]$DE$all.res %>% 
                                  filter(!is.na(!!as.name(fcCol)), !is.na(GeneSymbol))
print(paste0("nrow allRes = ", nrow(allRes)))
                        success <- bic.run.gsea.preranked(allRes,
                                                          fcCol,
                                                          gsea.sh,
                                                          compName,
        ...
        if(report){
            ...
            tryCatch({
                ...
                rpt <- bic.deseq.report(all_results, request, svnrev, fileName, pre = pre, 
                                        ...
                                        qcDir = out.dirs[[compSet]]$QCdir, fitType = fitType,
                                        annColors = sColors)
                log_info("Done.\n")
              }, error = function(e){
                print(str(e))
