...
my $proj_file_dir = "project_files";
...
GetOptions ('map=s' => \$map,
	    ...
	    'r2adaptor=s' => \$r2adaptor,
 	    ...
 	    'priority_group=s' => \$priority_group,
            ...
            'resume_deseq' =>\$resume_deseq,
            'save_space' => \$save_space) or exit(1);
...
if(!$map || !$species || !$strand || !$config || !$request || $help){
	...
	* OUTPUT: output results directory (default: results)
        ...
        * SAVE_SPACE: in order to save space, remove intermediate alignment files as soon as they are no longer needed (-save_space) 
HELP
exit;
}
...
if($ENV{'LSF_ENVDIR'} eq "/common/lsf/conf"){
    ...
    die "unrecognized scheduler, valid scheduler [sge, luna, juno, lilac]";
}
...
$proj_file_dir = "$output/$proj_file_dir";
`/bin/mkdir -m 775 -p $proj_file_dir`;                                          
`/bin/cp $request $proj_file_dir`;
...
if($deseq || $clustering_only){
    ...
    foreach my $sample_key (@sample_keys){
        ...
        close SK;
        `/bin/cp $sample_key $proj_file_dir`;

        ## check for corresponding comparisons file
        (my $compfile = $sample_key) =~ s/_key/_comparisons/;
        if(!-e $compfile){
            die "Can't find comparisons file corresponding to key file $sample_key";
        } 
        `/bin/cp $compfile $proj_file_dir`;
    }
}
...
if($species =~ /human|^hg19$/i){
    ...
    $GMT_DIR = "$Bin/data/human/MSigDB/v2023.1";
    if($lncrna){
    ...
    $GMT_DIR = "$Bin/data/mouse/MSigDB/v2023.1";
    if($r1adaptor){
    ...
    $GTF = '/lila/data/bicgrp/share/depot/annotation/S.cerevisiae/ensembl/v94/Saccharomyces_cerevisiae.R64-1-1.94.gtf';
...
my $standardParams = Schedule::queuing(%stdParams);
        ...
        if($save_space && $ran_sp){
            # remove if present
            my @files_to_rm = ("$output/intFiles/$sample/star2passGG", "$output/intFiles/$sample/$sample\_STAR_2PASS_Aligned.out.sam", glob("$output/intFiles/$sample/$sample\_STAR_1PASS*"));
            cleanup_files(join(" ", @files_to_rm), $spj, "$pre\_$uID\_STAR_$sample\_CLEANUP");
        }
	...
	if(!-e "$output/progress/$pre\_$uID\_STAR_AORRG_$sample.done" || $ran_sp){
	    sleep(3);
            ...
            my %addParams = (scheduler => "$scheduler", runtime => "160:00", rerun => "1", iounits => "1");
            my $additionalParams = Schedule::additionalParams(%addParams);
	    ...
	    my $standardParams = Schedule::queuing(%stdParams);
	`$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams $singularityParams $JAVA/java -Xms256m -Xmx20g -XX:-UseGCOverheadLimit -Djava.io.tmpdir=/scratch/$uID -jar $PICARD/picard.jar AddOrReplaceReadGroups I=$starOut O=$output/gene/alignments/$pre\_$sample\.bam SORT_ORDER=coordinate VALIDATION_STRINGENCY=LENIENT TMP_DIR=/scratch/$uID CREATE_INDEX=true RGID=$sample\_1 RGLB=_1 RGPL=Illumina RGPU=$sample\_1 RGSM=$sample`;
	...
	if($htseq || $dexseq){
	    ...
	    if(!-e "$output/progress/$pre\_$uID\_QNS_STAR_$sample.done" || $ran_sp){
            ...
            if($save_space && $ran_starqns){
                # remove if present
                cleanup_files($starOut, $starqnsj, "$pre\_$uID\_QNS_STAR_$sample\_CLEANUP");
            }
		...
		if(!-e "$output/progress/$pre\_$uID\_HT_STAR_$sample.done" || $ran_starqns){
		    ...
		    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_HT_STAR_$sample", job_hold => "$starqnsj", cpu => "1", mem => "4", cluster_out => "$output/progress/$pre\_$uID\_HT_STAR_$sample.log");
		    my $standardParams = Schedule::queuing(%stdParams);
	...
	if($kallisto || $rsem){
		...
		if(!-e "$output/progress/$pre\_$uID\_RSEM_PLOT_$sample.done" || $ran_rsem_exp){
		    ...
		    my $standardParams = Schedule::queuing(%stdParams);
                    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams $R/Rscript $RSEM/rsem-plot-model $output/transcript/rsem/counts_trans/$sample/$sample\_RSEM $output/metrics/rsem/$pre\_$sample\_RSEM.pdf`;
...
if($clustering_only){
	...
	if(!-e "$output/progress/$pre\_$uID\_DESeq_STAR.done" || $ran_shmatrix){
	    sleep(3);
	    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_DESeq_STAR", job_hold => "$shmatrixj", cpu => "6", mem => "36", cluster_out => "$output/progress/$pre\_$uID\_DESeq_STAR.log");
	    my $standardParams = Schedule::queuing(%stdParams);
...
sub cleanup_files {
    my $files_list_str = shift;
    my $job_holds = shift;
    my $job_name = shift;

    my %stdParams = (scheduler => "$scheduler", job_name => $job_name, job_hold => $job_holds, cpu => "1", mem => "1", cluster_out => "$output/progress/$job_name.log");
    my $standardParams = Schedule::queuing(%stdParams);
    my %addParams = (scheduler => "$scheduler", runtime => "500", priority_project=> "$priority_project", priority_group=> "$priority_group", queues => "lau.q,lcg.q,nce.q", rerun => "1", iounits => "1");
    my $additionalParams = Schedule::additionalParams(%addParams);

    print "\n CLENAUP CMD \n$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams $singularityParams $PERL/perl $Bin/cleanup_files.pl $files_list_str\n";
    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams $singularityParams $PERL/perl $Bin/cleanup_files.pl $files_list_str`;

    # This progress file does nothing if you delete it. Not sure if we need it, but maybe it would be nice to know at least one clean up happened..?
    `/bin/touch $output/progress/$pre\_$uID\_CLEANUP.done`;
}
...
sub rsyncJob{
    ...
    `$standardParams->{submit} $standardParams->{job_name} $standardParams->{job_hold} $standardParams->{cpu} $standardParams->{mem} $standardParams->{cluster_out} $additionalParams /usr/bin/rsync -azvP --exclude 'intFiles' --exclude 'progress' --exclude '*htseq_count' --exclude '*geneSymbols.txt' --exclude 'images' --exclude '*.rda' --exclude 'metrics/*.txt' --exclude 'crm' --exclude 'cell_composition' --exclude 'figures'  $curDir $rsync`;
...
#                die "CAN'T FIND cibersort IN $conf[1] $!";
    ...
    print "singularityBind = $singularityBind\n";
